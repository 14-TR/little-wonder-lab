...
- Use strict RED–GREEN–REFACTOR with real tests. The engineer is never their own reviewer. Use fresh CLI role sessions through the supervised, bounded `scripts/run_role.py` transport; code/accessibility and curriculum/safety reviewers run separately against the exact head and base SHA. The lead records observed runtime session IDs and validates process exit, closed-session evidence and complete role reports; an asynchronous dispatch handle alone is not completion. Never invent session IDs. This permits only foreground, bounded role processes under the existing supervision and time budgets; no detached work or weakened release safeguards.
