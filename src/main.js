...
function lessonCard(lesson, featured = false) {
  return `<a class="lesson-card ${featured ? 'featured' : 'shelf-card'}" href="#lesson/${escape(lesson.id)}" data-lesson-link="${escape(lesson.id)}">
    <div class="art-wrap ${escape(lesson.kind)}"><span class="folio">FIELD NOTE ${String(lesson.order).padStart(2, '0')}</span>${illustration(lesson.kind)}<span class="art-label">${escape(lesson.kind === 'shadows' ? 'A little light. A big discovery.' : lesson.kind === 'ramps' ? 'Let the good times roll.' : lesson.kind === 'patterns' ? 'Find the little group. Follow its rule.' : 'Think it. Fold it. Test it.')}</span></div>
