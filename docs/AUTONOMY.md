...
## Resuming across a maintenance base advance

A saved unfinished lesson can predate an owner maintenance release. Do not rewrite its historical plan/report base or discard/reimplement its dirty work. For an **unmerged** planned/engineered/reviewed item, the supervised lead may give a fresh planner spec both `base` (current operator main) and `checkpoint_base` (the literal saved checkpoint base). This optional field is planner-only: item/lesson still match, the operator HEAD must equal the new base, and the old base must be its verified ancestor. The helper leaves the checkpoint and original reports unchanged and retains all time, completion and final identity checks. Engineers/reviewers still require exact current checkpoint base; merged/live items use the existing provenance-gated recovery, never this path.

Before planning, save the entire original checkpoint to a new exclusive private JSON archive and record its hash. After a **fresh completed ready plan**, recheck main still matches that plan. Reuse the same owned worktree and compare changed paths before bringing in main. When maintenance paths and dirty lesson paths are disjoint and the branch is an ancestor, `git merge --ff-only origin/main` in that worktree preserves the dirty lesson without a stash, reset or interim lesson commit. Compare all original dirty-file bytes/hashes afterward. If fast-forward is unavailable, dirty paths overlap or identity changes, stop for separately reviewed integration; do not force, recreate, clean or automatically resolve conflicts.

Only then use the existing supervised `checkpoint --file` command for a new active planned checkpoint, with the same item/lesson/branch/worktree, current base, fresh plan/planner and current lead identities, and `prior_checkpoint_file` linking the complete immutable prior snapshot. Keep any same-item PR binding; retain stale head/engineer/review/evidence fields in the prior snapshot rather than treating them as current acceptance. Re-read the active checkpoint. A fresh engineer must finish the preserved work and supply a complete handoff; all tests, exact-SHA reviews and release/live gates run anew. Neither this transition nor its offline Git/SQLite tests grants daily credit, restores quota, proves model engineering sufficiency or resumes either paused cron.
...
All phase bounds include startup, final response and export. Engineer freezes implementation by 210 seconds, saves a complete report by 300, returns by 330, within the SAME 390-second allocation. Planner and reviewers save by 270 and return by 300 within 360 seconds. The final 60 seconds reserve transport, not extra implementation. Shorter absolute deadlines shorten work, never downstream gates or safe-stop reserves. These are transport and instruction contracts, not evidence that a real autonomous release succeeded.

The installed sequential and concurrent tool executor deadlines were observed at **420 seconds**, independent of terminal's requested timeout. The old 555-second engineer terminal call was abandoned at that ceiling before its role handoff. Use a **405-second engineer foreground terminal** (375 for planner/reviewers), never background work or a global timeout override. Its role deadline is at most 390 seconds, leaving five seconds containment plus ten seconds caller return inside 405, and fifteen seconds before 420. `validate_spec` rejects oversized windows instead of silently clamping an old spec. It also enforces the supervisor's per-invocation `LWL_LEAD_DEADLINE` (set before CLI startup, never reset by the lead): planner needs 1980 seconds remaining, engineering 1620, each reviewer 900. Admission runs again after Git/document preflight at dispatch. Missing/nonfinite clocks or insufficient reserves fail before model launch. This clock is a trusted supervision contract, not a sandbox against arbitrary environment writers. Tests exercise these real admission boundaries and finite subprocess completion/failure, not just prompt arithmetic. A bounded read-only model transport probe is not a production contribution or proof the shorter engineer implementation window suffices; parent-owned commissioning and all independent release gates remain necessary.
