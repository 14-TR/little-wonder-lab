...
The lead's sequential allocation is selection 60 seconds, planner 360, engineering plus lead gates 780 (600 engineer / 180 lead), independent reviews 360, release 420 and stop buffer 120: 2100 seconds total. The 2400-second outer limit is not extra working time. Check elapsed time at dispatch and irreversible boundaries; retain the release/stop reserves and stop when the next full stage cannot fit. Baseline work may overlap planning, never replace post-change gates. The optional repair and two new reviews require real remaining slack, not a larger total budget.
...
- `recover-merged --pr N`: read-back-only on GitHub; recovers a same-item merged PR into checkpoint. Use after an interrupted merge or a crash between live-checkpoint and final receipt.
...
- **Merged but no live receipt:** only recover that merge/deployment. If checkpoint stage is live after a crash, `recover-merged` then rerun `verify-live`. Deployment delay, stale cache, missing manifest, failed browser or newer unexpected deployment => keep pending and report, no second lesson and no fake success.
